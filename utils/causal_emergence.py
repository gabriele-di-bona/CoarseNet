...
https://www.hindawi.com/journals/complexity/2020/8932526/
...
def causal_emergence(G, span=-1, thresh=1e-4, t=500,
                     types=False, check_inacc=False, printt=False,
                     dev=False):
    r"""
    ...
    macro_labels = np.unique(sorted(macro_mapping.values()))
    G_macro_out = nx.relabel_nodes(G_macro,
                                    dict(zip(list(G_macro.nodes()),
                                             macro_labels)))
    CE['G_macro'] = G_macro_out
    CE['G_micro'] = G_micro
    ...
    if dev:
        ...
        wel = nx.to_pandas_edgelist(G_macro_out)
        mapping_df = pd.DataFrame({"micro":list(macro_mapping.keys()),
