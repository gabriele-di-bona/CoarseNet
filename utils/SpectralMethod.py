...
def Spectral(A):
    ...
    # Stochastic Random Walks Matrix
    normalization = np.sum(A, axis=0)
    W = A/normalization

    # We compute the left/right eigenvectors of W
    eigenvalues, left_eigenvectors, right_eigenvectors = scipy.linalg.eig(W, 
                                                                    left = True,
                                                                    right = True)
    ...
    # Let's divide the left eigenvector in 20 equal-size intervals
    I = 2
    ...
    for n_eig in range(n_relevant_eigenvectors):
        ...
        while sum(labels[:,n_eig]==0)!=0:
            super_node = (relevant_eigenvectors_l[:,n_eig]>=priors[n_eig])*(relevant_eigenvectors_l[:,n_eig]<=posteriors[n_eig])
            labels[super_node,n_eig] = label
            label = label+1
